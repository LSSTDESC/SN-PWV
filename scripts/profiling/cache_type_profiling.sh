...
export OUTPUT_DIR="RUN_$((SLURM_ARRAY_TASK_ID))"
mkdir OUTPUT_DIR
cd OUTPUT_DIR
...
python -m cProfile -o "cache_$SNAT_SIM_CACHE_TYPE.pstat" ../fitting_cli.py \
  --sim_pool_size $SIM_PROCESSES \
  --fit_pool_size $FITTING_PROCESSES \
  --iter_lim $NUMBER_SIMS \
  --cadence $USE_CADENCE \
  --sim_variability $SIM_VARIABILITY \
  --fit_variability $FIT_VARIABILITY \
  --out_path "cache_$SNAT_SIM_CACHE_TYPE.csv"
...
python -m cProfile -o "cache_second_$SNAT_SIM_CACHE_TYPE.pstat" ../fitting_cli.py \
  --sim_pool_size $SIM_PROCESSES \
  --fit_pool_size $FITTING_PROCESSES \
  --iter_lim $NUMBER_SIMS \
  --cadence $USE_CADENCE \
  --sim_variability $SIM_VARIABILITY \
  --fit_variability $FIT_VARIABILITY \
  --out_path "cache_second_$SNAT_SIM_CACHE_TYPE.csv"
